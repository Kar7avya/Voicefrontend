...
import { getCurrentUser, getAuthHeaders } from './supabaseClient';
...
const BACKEND_URL = process.env.REACT_APP_BACKEND_URL || 'https://voicebackend-development.onrender.com';
  ...
  const checkAuth = async () => {
    try {
      ...
      if (currentUser && currentUser.id) {
        ...
        toast.warning('Please log in to view your presentations');
      }
    } catch (error) {
  ...
  const fetchMetadata = async () => {
    try {
      ...
      console.log('🔍 Fetching metadata with auth headers...');
      ...
      console.log(`✅ Loaded ${metadataList.length} presentations`);
        ...
        <MainContent>
          <AuthWarning>
            ...
            <p style={{ marginTop: '1rem', marginBottom: '1rem' }}>
              Please log in to view your presentation dashboard and analysis history.
            </p>
                ...
                {metadataList.map(item => {
                  ...
                  return (
                    <DataCard key={item.id}>
                      <CardHeader>
                        <CardTitle>🎥 {item.file_name || item.video_name || 'Untitled Presentation'}</CardTitle>
