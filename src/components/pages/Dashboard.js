...
function Dashboard() {
  ...
  const analyzePerformance = (item) => {
    ...
    const totalWords = wordsArray.length;
    const fluencyScore =
      totalWords > 0 ? Math.round(((totalWords - fillerWordsCount) / totalWords) * 100) : 0;

    let speakingRate = 0;
    if (totalWords > 0 && videoDuration > 0) {
      const durationInMinutes = videoDuration / 60;
      speakingRate = Math.round(totalWords / durationInMinutes);
    } else if (totalWords > 0 && item.video_duration) {
      const durationInMinutes = item.video_duration / 60;
      speakingRate = Math.round(totalWords / durationInMinutes);
    }
    ...
    const getFluencyRating = (score) => {
      if (score >= 90)
        return {
          variant: "success",
          text: "Excellent",
          explanation: "Your speech is clear with minimal filler words!",
        };
      if (score >= 70)
        return {
          variant: "warning",
          text: "Good",
          explanation: "Your speech is mostly clear. Try reducing filler words.",
        };
      return {
        variant: "danger",
        text: "Needs Work",
        explanation: 'Focus on reducing "um", "uh", and other filler words.',
      };
    };

    const getFillerRating = (count, total) => {
      if (total === 0) return { variant: "success", text: "Excellent" };
      const percentage = (count / total) * 100;
      if (percentage < 5) return { variant: "success", text: "Excellent" };
      if (percentage < 10) return { variant: "warning", text: "Good" };
