...
export default function Upload() {
  ...
  const handleFileSelect = (selectedFile) => {
    ...
    if (!allowedTypes.includes(selectedFile.type)) { toast.error("Invalid File Type: .mp4, .mov, .avi, .mp3, .wav only."); return; }
    setFile(selectedFile);
  };
  ...
  const handleUpload = useCallback(async (e) => {
    if (e?.preventDefault) e.preventDefault();
    if (!user || !user.id || !user.id.includes("-")) { toast.error("Authentication error."); navigate("/login"); return; }
    ...
    try {
      ...
      if (file.type.startsWith("video/")) {
        try {
          const extractForm = new FormData();
          extractForm.append("videoName", uploadData.videoName);
          const extractRes = await fetch(`${BACKEND_URL}/api/extractFrames`, { method: "POST", headers: authHeaders, body: extractForm });
          if (extractRes.ok) {
            setProgress(50); setProgressLabel("Analyzing visuals...");
            const analyzeRes = await fetch(`${BACKEND_URL}/api/analyzeAllFrames`, { headers: authHeaders });
            if (analyzeRes.ok) {
              const analyzeData = await analyzeRes.json();
              const frames = Array.isArray(analyzeData) ? analyzeData : analyzeData.responses || [];
        ...
        } catch { toast.warn("Frame processing error."); }
      }

      setProgress(65); setProgressLabel("Transcribing with ElevenLabs...");
      try {
        const elevenForm = new FormData();
        elevenForm.append("videoName", uploadData.videoName);
        const elevenRes = await fetch(`${BACKEND_URL}/api/transcribeWithElevenLabs`, { method: "POST", headers: authHeaders, body: elevenForm });
        if (elevenRes.ok) {
          const elevenData = await elevenRes.json();
          setElevenLabsTranscript(elevenData.transcript || "No transcript");
          toast.success("ElevenLabs transcription done!");
        }
      } catch { toast.warn("ElevenLabs error."); }

      setProgress(80); setProgressLabel("Transcribing with Deepgram...");
      try {
        const deepgramForm = new FormData();
        deepgramForm.append("videoName", uploadData.videoName);
        const deepgramRes = await fetch(`${BACKEND_URL}/api/transcribeWithDeepgram`, { method: "POST", headers: authHeaders, body: deepgramForm });
        if (deepgramRes.ok) {
          const deepgramData = await deepgramRes.json();
          const transcript = deepgramData.transcript || "No transcript";
          setDeepgramTranscript(transcript);
          toast.success("Deepgram transcription done!");

          if (transcript && transcript !== "No transcript from Deepgram") {
            setProgress(90); setProgressLabel("Analyzing with Gemini AI...");
            try {
              const analysisRes = await fetch(`${BACKEND_URL}/api/analyzeSpeechWithGemini`, {
              ...
              if (analysisRes.ok) {
                const analysisData = await analysisRes.json();
                setLlmAnalysisResult(analysisData.analysis || "No analysis");
                toast.success("AI analysis complete!");
              }
            } catch { toast.error("Gemini analysis failed."); }
          }
        }
      } catch { toast.warn("Deepgram error."); }
  ...
  const handleManualTextAnalysis = async () => {
    ...
    if (!manualTranscript.trim()) { toast.error("Please enter text."); return; }
    setLoading(true); setLlmAnalysisResult(""); setProgress(20); setProgressLabel("Analyzing with Gemini...");
    try {
      const authHeaders = await getAuthHeaders();
      const analysisRes = await fetch(`${BACKEND_URL}/api/analyzeSpeechWithGemini`, {
        method: "POST", headers: { "Content-Type": "application/json", ...authHeaders },
      ...
      if (!analysisRes.ok) throw new Error(analysisRes.statusText);
      const analysisData = await analysisRes.json();
      setLlmAnalysisResult(analysisData.analysis || "No analysis");
      setProgress(100); toast.success("Analysis complete!");
    } catch (error) { toast.error(`Analysis failed: ${error.message}`); }
    ...
    return (
      <div className="min-h-screen bg-[#080810] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/50 text-sm tracking-widest uppercase">Verifying session</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-[#080810] flex items-center justify-center">
        <div className="text-center max-w-md w-full px-8">
          ...
          <div className="relative w-24 h-24 mx-auto mb-8">
            <div className="absolute inset-0 rounded-full bg-gradient-to-br from-violet-600 to-blue-500 animate-pulse opacity-30" />
            <div className="absolute inset-2 rounded-full bg-gradient-to-br from-violet-500 to-blue-400 animate-ping opacity-20" />
            <div className="absolute inset-4 rounded-full bg-gradient-to-br from-violet-400 to-cyan-400 flex items-center justify-center">
          ...
          <h3 className="text-white text-xl font-semibold mb-2">{progressLabel}</h3>
          <p className="text-white/40 text-sm mb-6">Processing your presentation with AI</p>
  ...
  return (
    <div className="min-h-screen bg-[#080810] pt-28 pb-16 px-4 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-4 py-1.5 text-xs text-white/50 uppercase tracking-widest mb-6">
          ...
          <p className="text-white/40 text-lg">
            Get instant AI feedback on speech, delivery, and visuals.
          </p>
        </div>
        ...
        {!user && (
          <div className="mb-6 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center gap-3">
            ...
            <p className="text-amber-300/80 text-sm">
              ...
              <button onClick={() => navigate("/login")} className="text-amber-400 underline underline-offset-2 font-medium">
                Sign in
              </button>{" "}
              to upload files and save analysis history.
            </p>
          </div>
        )}

        {user && (
          <div className="mb-6 p-4 rounded-2xl bg-green-500/8 border border-green-500/15 flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-green-500/20 flex items-center justify-center flex-shrink-0">
              <span className="text-green-400 text-sm">✓</span>
            </div>
            <div>
              <p className="text-green-400 text-sm font-medium">Signed in</p>
        ...
        <div
          ...
          onDrop={handleDrop}
          className={`
            relative group cursor-pointer rounded-3xl border-2 border-dashed p-16 text-center mb-6
            transition-all duration-300 ease-out
            ${dragOver
              ? "border-violet-400 bg-violet-500/10 scale-[1.01]"
              : user
                ? "border-white/10 hover:border-violet-500/50 hover:bg-white/[0.02] bg-white/[0.01]"
        ...
        >
          ...
          <div className={`
            ...
            ${dragOver ? "bg-violet-500/30 scale-110" : "bg-white/5 group-hover:bg-violet-500/15 group-hover:scale-105"}
          `}>
            <svg className={`w-9 h-9 transition-colors duration-300 ${dragOver ? "text-violet-300" : "text-white/30 group-hover:text-violet-400"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
            </svg>
          </div>

          <h3 className="text-white text-xl font-semibold mb-2">
            {dragOver ? "Release to upload" : user ? "Drop your file here" : "Login required"}
          </h3>
          <p className="text-white/30 text-sm mb-4">or click to browse files</p>
          <div className="inline-flex items-center gap-2 bg-white/5 rounded-full px-4 py-2">
            <span className="text-white/30 text-xs">MP4 · MOV · AVI · MP3 · WAV</span>
            <span className="w-px h-3 bg-white/10" />
            <span className="text-white/30 text-xs">Max 500MB</span>
          </div>
        ...
        <div className="flex justify-center mb-6">
          <button
            onClick={() => setShowTextArea((prev) => !prev)}
            disabled={!user}
            className="inline-flex items-center gap-2 text-white/40 hover:text-white/70 text-sm transition-colors duration-200 disabled:opacity-30 disabled:cursor-not-allowed"
          >
            <span>{showTextArea ? "Hide" : "Paste text instead"}</span>
            <svg className={`w-4 h-4 transition-transform duration-200 ${showTextArea ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Text area */}
        {showTextArea && (
          <div className="mb-8 rounded-3xl border border-white/8 bg-white/[0.02] p-6">
            <label className="text-white/50 text-xs uppercase tracking-widest block mb-3">Paste Transcript</label>
            <textarea
              rows={6}
              placeholder={user ? "Paste your speech or transcript here for AI analysis..." : "Login required"}
              value={manualTranscript}
              onChange={(e) => setManualTranscript(e.target.value)}
              disabled={!user}
            ...
            <div className="flex justify-end mt-4 pt-4 border-t border-white/5">
              <button
                onClick={handleManualTextAnalysis}
                disabled={loading || !manualTranscript.trim() || !user}
                className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-blue-600 text-white text-sm font-medium
                  hover:from-violet-500 hover:to-blue-500 transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed
        ...
        {hasResults && (
          <div className="space-y-4">
            <h2 className="text-white/60 text-xs uppercase tracking-widest font-medium px-1">Analysis Results</h2>

            {publicUrl && (
              <ResultCard icon="🔗" title="File URL">
                <a href={publicUrl} target="_blank" rel="noopener noreferrer"
                  className="text-violet-400 hover:text-violet-300 text-sm truncate block transition-colors">
                  {filename}
                </a>
              </ResultCard>
            )}
            ...
            {responses.length > 0 && (
              <ResultCard icon="👁" title="Visual Analysis">
                <ul className="space-y-2">
                  {responses.map((r, i) => (
                    <li key={i} className="text-white/50 text-sm leading-relaxed border-l-2 border-violet-500/30 pl-3">{r}</li>
                  ))}
                </ul>
              </ResultCard>
            )}
            ...
            {elevenLabsTranscript && (
              <ResultCard icon="🎙" title="ElevenLabs Transcript">
                <p className="text-white/50 text-sm leading-relaxed">{elevenLabsTranscript}</p>
              </ResultCard>
            )}

            {deepgramTranscript && (
              <ResultCard icon="🎤" title="Deepgram Transcript">
                <p className="text-white/50 text-sm leading-relaxed">{deepgramTranscript}</p>
              </ResultCard>
            )}

            {llmAnalysisResult && (
              <ResultCard icon="✨" title="Gemini AI Analysis" highlight>
                <p className="text-white/70 text-sm leading-relaxed whitespace-pre-wrap">{llmAnalysisResult}</p>
              </ResultCard>
            )}
          </div>
        )}
      ...
      <ToastContainer position="top-right" autoClose={5000} hideProgressBar={false} draggable theme="dark" />
    </div>
  );
}

// Reusable result card
function ResultCard({ icon, title, children, highlight }) {
  return (
    <div className={`rounded-2xl border p-6 transition-all duration-200
      ${highlight
        ? "border-violet-500/20 bg-gradient-to-br from-violet-500/5 to-blue-500/5"
        : "border-white/6 bg-white/[0.02]"
      }`}>
      <div className="flex items-center gap-2 mb-4">
        <span className="text-base">{icon}</span>
        <h3 className="text-white/70 text-xs uppercase tracking-widest font-medium">{title}</h3>
        {highlight && <span className="ml-auto text-[10px] text-violet-400 bg-violet-500/10 border border-violet-500/20 rounded-full px-2 py-0.5">AI Powered</span>}
      </div>
      {children}
    </div>
  );
